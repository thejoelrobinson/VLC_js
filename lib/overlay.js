...
export function update_overlay(overlay) {
  ...
  let progressPosition = Number(media_player.get_position()) * 100;
  ...
  const volume = Number(media_player.get_volume());
  ...
  // Set duration text
  // VLC 4.0 returns BigInt for time values — convert to Number for arithmetic
  let time = Number(media_player.get_time());
  let seconds = Math.trunc(time / 1000);
  let minutes = Math.trunc(seconds / 60);
  seconds = ('00'+seconds % 60).slice(-2);
  let max_time = Number(media_player.get_length());
