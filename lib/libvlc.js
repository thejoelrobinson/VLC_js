...
export class MediaPlayer {
  ...
  set_position(position, fast = false) {
    const result = this.module._wasm_media_player_set_position(this.media_player_ptr, position, fast);
    if (window._vlcStateCache) {
      window._vlcStateCache.position = position;
      if (window._vlcStateCache.lengthMs > 0) {
        window._vlcStateCache.timeMs = position * window._vlcStateCache.lengthMs;
      }
    }
    return result;
  }
  ...
  set_mute(mute) {
    if (window._vlcStateCache) window._vlcStateCache.muted = !!mute;
  ...
  set_volume(volume) {
    if (window._vlcStateCache) window._vlcStateCache.volume = volume;
