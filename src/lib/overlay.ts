...
interface OverlayLayout {
  VOLUME_BAR_WIDTH: number;
  VOLUME_BAR_HEIGHT: number;
  PROGRESS_TEXT_WIDTH: number;
  PROGRESS_BAR_WIDTH: number;
  PROGRESS_BAR_HEIGHT: number;
  PLAY_BUTTON_X: number;
  PROGRESS_BAR_X: number;
  PROGRESS_TEXT_X: number;
  VOLUME_BUTTON_X: number;
  VOLUME_BAR_X: number;
}

function get_layout(canvas: HTMLCanvasElement): OverlayLayout {
...
// Suppress unused variable warnings for image assets loaded at module scope
void PROGRESS_BAR_COLOR; void PROGRESS_BAR_BG_COLOR; void VOLUME_BAR_COLOR;
void VOLUME_BAR_MUTE_COLOR; void VOLUME_BAR_BG_COLOR; void BUTTON_BG_COLOR;
void GENERAL_BG_COLOR; void TEXT_COLOR; void MENU_BAR_HEIGHT;
void central_play_icon; void play_button; void pause_button;
void volume_button; void muted_button;

// Apply underlying changes (eg video is paused) to the displayed UI
export function update_overlay(): void {
  const media_player = window.media_player;
  if (media_player == null) {
    return;
  }

  // VLC 4.0 libvlc_media_player_is_playing() acquires vlc_player_Lock which
  // calls pthread_cond_timedwait — calling it from the main browser thread
  // triggers "Blocking on main thread" and WASM heap abort.
  // Use window._vlcIsPlaying (a cached JS state set by main.js play/pause events).
  const is_paused = !window._vlcIsPlaying;
  ...
  // In VLC 4.0, get_position/time/length all acquire vlc_player_Lock which
  // calls pthread_cond_timedwait — illegal on the browser main thread (causes
  // WASM heap abort). Use window._vlcStateCache instead, updated from
  // VideoFrame timestamps in _vlcOnDecoderFrame and from user interactions.
  const cache = window._vlcStateCache ?? {} as Partial<VlcStateCache>;
  let progressPosition = (cache.position ?? 0) * 100;
  ...
  progressCurrent.closest('[role="slider"]')?.setAttribute('aria-valuenow', String(Math.round(progressPosition)));

  // Volume and mute — from cache (set by set_volume/toggle_mute interactions)
  const isMuted = cache.muted ?? false;
  const volume = cache.volume ?? 80;
  progressCurrentVolume.style.setProperty('--current-audio-progress-width', `${isMuted ? 0 : volume}%`);
  progressCurrentVolume.closest('[role="slider"]')?.setAttribute('aria-valuenow', String(isMuted ? 0 : volume));
  ...
  // Time/length from cache (updated from VideoFrame timestamps during decode)
  // Use separate variables for numeric and formatted values to satisfy strict typing.
  const time = cache.timeMs ?? 0;
  const totalSeconds = Math.trunc(time / 1000);
  const minutes = Math.trunc(totalSeconds / 60);
  const seconds = ('00' + totalSeconds % 60).slice(-2);

  const max_time = cache.lengthMs ?? 0;
  const totalMaxSeconds = Math.trunc(max_time / 1000);
  const max_minutes = Math.trunc(totalMaxSeconds / 60);
  const max_seconds = ('00' + (totalMaxSeconds % 60)).slice(-2);

  timer.innerHTML = `${minutes}:${seconds} / ${max_minutes}:${max_seconds}`;

  if (is_paused && window.files) {
    ...
    bPlayButton.setAttribute('aria-label', 'Play');
  }
  ...
  if (!is_paused && window.files) {
    ...
    bPlayButton.setAttribute('aria-label', 'Pause');
  }

  const nextChapter = document.getElementById('next-chapter')!;
  const previousChapter = document.getElementById('previous-chapter')!;

  // Chapter info from cache (avoids vlc_player_Lock on main thread)
  const currentChapter = cache.chapter ?? -1;
  const chapterCount = cache.chapterCount ?? 0;
  if (chapterCount) {
    if (currentChapter > 0) {
...
export function on_overlay_click(_mouseEvent: MouseEvent): void {
